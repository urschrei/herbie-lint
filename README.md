...
Create a `Herbie.toml` file in your project root:

```toml
# Path to the SQLite database containing expression transformations
db_path = "path/to/Herbie.db"

# Whether to call the Herbie tool for unknown expressions
# - true: Require Herbie, fail if not found
# - false: Never call Herbie (default for most users)
use_herbie = false
...
To suppress the lint for a specific function or module, use the standard Rust lint control:
...
The lint uses a SQLite database containing pre-computed expression transformations. The database format is compatible with the [GHC Herbie Plugin](https://github.com/mikeizbicki/HerbiePlugin).

You can:
1. Use the included `db/Herbie.db` database
2. Generate new entries by enabling `use_herbie = true` (requires Herbie to be installed)
